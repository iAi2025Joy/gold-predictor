...
MODEL_ARTIFACTS_FILE = "oil_model_artifacts.json"  # the trained model's actual math, exported for live inference in the Node backend -- same mechanism already proven for gold
...
FEATURE_COLS = ["wti_ret_1d", "wti_ret_3d", "wti_ret_5d", "wti_ma_ratio", "wti_vol10", "rsi14",
                 ...
                 "oil_implied_vol", "implied_vol_data_available",
...
def run_prediction_pipeline(history_df):
    ...
    if len(df_model) < MIN_ROWS_FOR_PREDICTION:
        return {
            ...
            "latest_oil_implied_volatility_pct": None,
    ...
    # Extracted so oilLiveInference.js has a cached fallback source for
    # these three model inputs -- mirrors gold's exact same exposed
    # fields (latest_real_yield_10y_pct/latest_dxy/latest_vix). These
    # were already being fed to the model via FEATURE_COLS but were
    # never surfaced in the output JSON before -- a real gap that would
    # have silently broken live inference (no cached value to fall back
    # on when a feature isn't recomputed live).
    latest_real_yield_10y = float(latest_row["real_yield_10y"].values[0])
    latest_dxy_value = float(latest_row["dxy"].values[0])
    ...
    latest_oil_implied_vol = float(latest_row["oil_implied_vol"].values[0])
    latest_implied_vol_available = bool(latest_row["implied_vol_data_available"].values[0])
    implied_vol_range = (
        gpu.compute_implied_volatility_range(latest_oil_implied_vol, current_price, horizon_days=1)
    ...
    # Reuses gold's already-verified export_model_artifacts/export_tree
    # directly (via the gpu import already used elsewhere in this file)
    # rather than duplicating that serialization logic -- same trained-
    # model-math export mechanism, just fed oil's own model objects and
    # feature set. Lets a live oil price genuinely flow through the real
    # trained model on request, the same capability gold already has,
    # instead of oil's prediction always being the once-daily cached
    # result.
    model_artifacts = gpu.export_model_artifacts(
        scaler, FEATURE_COLS,
    ...
    return {
        ...
        "latest_dxy": latest_dxy_value if latest_macro_data_available else None,
        ...
        "latest_oil_implied_volatility_pct": latest_oil_implied_vol if latest_implied_vol_available else None,
        ...
        "implied_volatility_note": (
            ...
            "CBOE's Crude Oil ETF Volatility Index, computed from actual USO options prices, same "
            ...
            "predictions above (this system's own model outputs) -- never conflate the two."
        ),
        ...
        "_model_artifacts": model_artifacts,  # popped off and saved to a separate file in main() -- never meant to end up in oil_prediction_latest.json itself
    }
...
def main():
    ...
    # Popped BEFORE printing (matching gold's exact order) -- otherwise
    # the full tree-structure dump (potentially thousands of numbers)
    # would flood the workflow log.
