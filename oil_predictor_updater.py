...
def run_prediction_pipeline(history_df):
    ...
    if backtest_acc is None:
        ...
        if backtest_acc < baseline_acc:
            confidence_note = (
                f"This model's backtested accuracy ({backtest_acc:.1%}) was actually LOWER than simply "
                f"always predicting the majority class ({baseline_acc:.1%}) on real held-out test data -- "
                f"this model showed no real edge and underperformed even the trivial baseline. Treat any "
                f"direction call from it with strong skepticism, not just mild caution -- it is not a case "
                f"of 'close but unproven', it actively did worse."
            )
        else:
            confidence_note = (
