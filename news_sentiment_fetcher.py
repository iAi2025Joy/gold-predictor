...
---------------------------------
This script pulls one broad, unfiltered batch (up to 50 articles per call --
Alpha Vantage rejected an economy_macro/financial_markets topics filter as
"Invalid inputs", confirmed directly; see fetch_news_batch) and then applies
keyword filters over each
...
def fetch_news_batch():
    # A confirmed real bug this fixes: sending topics="economy_macro,financial_markets"
    # as an input filter made Alpha Vantage reject the whole request with
    # "Invalid inputs" -- confirmed directly by testing the real API key
    # both with and without this parameter. Without it: a real, successful
    # batch of 50 articles came back, including several genuinely tagged
    # "economy_macro" and "financial_markets" in their OWN results -- so
    # those are valid topic names for Alpha Vantage's own classification,
    # just not accepted the way this script was passing them as an input
    # filter. Rather than keep guessing at the exact input syntax Alpha
    # Vantage wants, this drops the topics filter entirely and fetches a
    # broad, unfiltered batch instead -- the script's own keyword-based
    # relevance filtering below (is_relevant/aggregate_sentiment) already
    # does the real narrowing work downstream regardless of what Alpha
    # Vantage's own topic categorization returns, so this is a safe,
    # low-risk fix, not a loss of real filtering capability.
    if not API_KEY:
