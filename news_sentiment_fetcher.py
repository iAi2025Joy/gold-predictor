...
def fetch_news_batch():
    ...
    if "Information" in data and "feed" not in data:
        # DIAGNOSTIC (added after a real, unreproducible "Invalid inputs"
        # failure -- confirmed via manual re-test that the request itself
        # (these exact params) is valid and returns real article data, so
        # the failure was transient: most likely the shared 25-requests/day
        # Alpha Vantage budget (split with oil_predictor_updater.py, see
        # module docstring) being hit at that moment, or a brief upstream
        # hiccup, rather than a real bug in this request. Alpha Vantage
        # doesn't return a consistent, distinguishable message for "rate
        # limited" vs "actually malformed" on every endpoint, so the ONLY
        # way to tell them apart after the fact is to have the full raw
        # response in the log -- the single "Information" string alone
        # doesn't say which case this was. This print does not change
        # behavior (the run still fails the same way via the raise below);
        # it only makes the next occurrence diagnosable without a manual
        # re-test like the one that resolved this incident.
        print(f"Alpha Vantage NEWS_SENTIMENT call did not return a feed. Full raw response for diagnosis: {json.dumps(data)}")
        print(f"Params sent (apikey redacted): {{'function': params['function'], 'topics': params['topics']}}")
