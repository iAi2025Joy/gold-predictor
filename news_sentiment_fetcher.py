...
def is_gold_relevant(article):
    # CORRECTION: an earlier version matched against title + summary combined.
    # Tested against a real batch, this produced false positives -- e.g.
    # "Casino Operator Las Vegas Sands..." and "Can American Airlines..."
    # matched, almost certainly because their summaries contained generic
    # boilerplate macro phrasing (e.g. "amid a challenging interest rate
    # environment") without the article actually being ABOUT anything
    # gold-relevant. Titles reflect an article's actual subject far more
    # reliably than summaries, which often include incidental surrounding
    # context. Matching on title only is a real precision fix, not a
    # cosmetic one.
    title = article.get("title", "").lower()
    return any(kw in title for kw in GOLD_RELEVANT_KEYWORDS)
