...
GOLD_RELEVANT_KEYWORDS = [
    ...
    # Same class of bug already fixed for "oil"/"china"/"fed" below --
    # just missed for this term originally.
    ...
    # Word-boundary matching alone can't fix this one -- "Powell" (Fed
    # Chair) and "Powell Industries" (ticker POWL, an unrelated company)
    # are both genuine standalone words, a true naming collision rather
    # than a substring-inside-a-word problem. See POWELL_COMPANY_EXCLUSIONS
    # and is_gold_relevant() below for the actual fix.
...
WORD_BOUNDARY_KEYWORDS = ["fed", "oil", "china", "chinese", "gold", "powell"]

# Explicit exclusions for genuine whole-word name collisions that
# word-boundary matching cannot distinguish on its own -- "Powell"
# (Fed Chair Jerome Powell) and "Powell Industries" (ticker POWL, an
# unrelated industrial company) are both real, standalone words.
# Confirmed via real data: "Powell Industries (POWL) Registers a Bigger
# Fall Than the Market" was incorrectly matched as gold-relevant.
POWELL_COMPANY_EXCLUSIONS = ["powell industries"]
...
def is_gold_relevant(article):
    ...
    if any(excl in title for excl in NON_PETROLEUM_OIL_EXCLUSIONS):
        return False
    if any(excl in title for excl in POWELL_COMPANY_EXCLUSIONS):
        return False
...
def aggregate_sentiment(articles):
    ...
    # can return the same article twice in one batch (e.g. "Why China
    # Could Become Costco's Next Membership Catalyst" appeared twice in a
    # single pull), which without this step gets double-counted toward
    # articles_matched and double-weighted in the average sentiment score.
    seen_titles = set()
    deduped = []
    for a in relevant:
        t = a.get("title", "")
