...
# load_fed_rate_outlook, get_current_fed_funds_range, and
# get_market_rate_expectation_proxy now live in gold_predictor_updater.py
# (imported above as gpu) and are called as gpu.<name>() throughout this
# file -- moved there so gold_predictor_updater.py can also use them for
# its own model (Fed rate features feeding gold's prediction), without
# duplicating this logic in two places or creating a circular import.
...
def load_dxy_history():
    ...
    # A confirmed real bug this fixes: FRED's DTWEXBGS (the series behind
    # "DXY" here) only updates ONCE per business day, but this data is
    # collected every 6 hours -- so without this step, ~4 rows per day
    # all carry the exact same DXY value until the next real daily update
    # lands. When add_dxy_features() computes day-over-day returns from
    # those intraday duplicates, most of them show a 0% change, and
    # "next_ret > 0" treats "no change" the same as "down" (both land in
    # class 0) -- artificially inflating the down/flat class. With only
    # ~60 rows total early in this pipeline's life, that was enough to
    # occasionally make an ENTIRE training split contain zero real "up"
    # examples, crashing LogisticRegression.fit with "needs samples of at
    # least 2 classes". Resampling to one row per calendar day (the last
    # real reading of that day, which reflects FRED's actual daily value)
    # removes the artificial duplication at the source, instead of just
    # deduping identical exact timestamps (which the old
    # drop_duplicates(subset="Date") below did -- that never catches this,
    # since each 6-hourly fetch has a genuinely different timestamp even
    # though the underlying DXY value hasn't actually changed yet).
    df["_day"] = df["Date"].dt.date
    df = df.sort_values("Date").drop_duplicates(subset="_day", keep="last").drop(columns=["_day"])
...
def run_prediction_pipeline(dxy_history_df):
    ...
    if len(df_model) < MIN_ROWS_FOR_PREDICTION:
        fed_funds_lower, fed_funds_upper = gpu.get_current_fed_funds_range()
        fed_rate_outlook = gpu.load_fed_rate_outlook()
        market_rate_proxy = gpu.get_market_rate_expectation_proxy()
        return {
            ...
            "market_rate_expectation_proxy": market_rate_proxy,
            "fed_rate_outlook_note": (
                "fed_rate_outlook contains the Fed's OWN published median rate projections (the "
                "'dot plot'), NOT market-implied probability. market_rate_expectation_proxy is a "
                "SEPARATE, rough approximation of market pricing using 2yr Treasury yields -- see its "
                "own methodology_note. Always attribute correctly. Both fields work independently of "
                "the DXY prediction model above, so they're available even while that model is still "
                "gathering data."
            ),
    ...
    # Defensive guard against a real, confirmed crash: LogisticRegression
    # (and, less obviously, GradientBoostingClassifier's own internal
    # checks) require at least 2 distinct classes in the training labels.
    # The data-level fix above (one row per calendar day) removes the
    # main known cause of this, but rather than trust that alone, this
    # fails gracefully with an honest result instead of crashing the
    # whole scheduled job and silently leaving dxy_prediction_latest.json
    # stale for a full cycle -- consistent with this project's standard
    # of honest degradation over a hard crash when real data is thin.
    if y_train.nunique() < 2:
        fed_funds_lower, fed_funds_upper = gpu.get_current_fed_funds_range()
        return {
            "prediction": "insufficient_data",
            "confidence_note": (
                "The training data collected so far shows no genuine day-to-day direction variation "
                "yet (every day in the training window moved the same way, or was flat) -- a real "
                "model can't be trained on a single class. This resolves as more varied real daily "
                "data accumulates; it is not an error in the pipeline itself."
            ),
            "model_type_used": None,
            "current_dxy": float(dxy_history_df["DXY"].iloc[-1]) if len(dxy_history_df) else None,
            "predicted_dxy": None,
            ...
            "price_direction_vs_current": None,
            "direction_price_agreement": None,
            "price_confidence_note": "Not enough varied data yet to make a forecast.",
            "is_price_prediction_significant": False,
            "model_accuracy_vs_baseline": None,
            "is_statistically_significant": False,
            "current_fed_funds_rate_lower_pct": fed_funds_lower,
            "current_fed_funds_rate_upper_pct": fed_funds_upper,
            "fed_rate_outlook": gpu.load_fed_rate_outlook(),
            "market_rate_expectation_proxy": gpu.get_market_rate_expectation_proxy(),
            "historical_data_start_date": dxy_history_df["Date"].min().strftime("%Y-%m-%d") if len(dxy_history_df) else None,
            "historical_data_end_date": dxy_history_df["Date"].max().strftime("%Y-%m-%d") if len(dxy_history_df) else None,
        }
    ...
    if backtest_acc is None:
        ...
        if backtest_acc < baseline_acc:
            confidence_note = (
                ...
                f"trivial baseline. Treat any direction call from it with strong skepticism."
            )
        else:
            confidence_note = (
    ...
    return {
        ...
        "fed_rate_outlook_note": (
            ...
            "market_rate_expectation_proxy is a SEPARATE, rough approximation of market pricing using "
            "2yr Treasury yields -- see its own methodology_note field. Always attribute each correctly."
        ),
