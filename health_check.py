...
def check_freshness(filename, label):
    try:
    ...
    if age_hours > STALE_THRESHOLD_HOURS:
        return False, (
            f"{label}: data is {age_hours:.1f} hours old (threshold: {STALE_THRESHOLD_HOURS}h) -- "
            f"the hourly update job may have stopped running or is failing."
        )
