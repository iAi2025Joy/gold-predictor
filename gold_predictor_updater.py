...
API_KEY = os.environ.get("GOLD_API_KEY", "")   # your GoldAPI.io key
...
# ============================================================
# 1. LIVE PRICE (confirmed format against the real GoldAPI.io dashboard)
# ============================================================

def fetch_live_price():
    """Fetch the current gold spot price from GoldAPI.io.

    CORRECTION: an earlier version of this script switched to Binance's
    public PAXG/USDT price for this recurring fetch, to avoid GoldAPI's
    100-requests/month cap and allow hourly updates. That was a mistake:
    Binance blocks automated/datacenter traffic (confirmed directly --
    both the one-time historical backfill AND this live fetch failed
    with HTTP 451 "Unavailable For Legal Reasons" when run from GitHub
    Actions' servers). The one-time backfill worked around this by
    running from a personal computer instead; this recurring fetch runs
    on GitHub Actions every time, so it cannot use Binance at all. Back
    to GoldAPI here, which is confirmed reliable from GitHub Actions --
    at the cost of returning to an every-8-hours schedule instead of
    hourly, to stay within its free-tier request budget."""
    if not API_KEY:
