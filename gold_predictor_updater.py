...
MODEL_ARTIFACTS_FILE = "gold_model_artifacts.json"  # the trained model's actual math, exported for live inference in the Node backend
...
def merge_macro_data(price_df, macro_file="macro_history.json"):
    ...
    ALSO attaches Fed funds rate / 2yr yield features (added later than
    the original 3 series) as a SEPARATE availability flag
    (fed_rate_data_available), not folded into macro_data_available --
    those two new series weren't collected in the earliest macro_history
    rows, so requiring them for the original flag would retroactively
    change already-tested behavior on old data. fed_funds_midpoint gives
    the model the actual current policy rate level (a real driver: lower
    real yields/rates generally support gold); yield_fed_spread (2yr
    yield minus the funds midpoint) is a more dynamic, daily-moving
    signal than the funds rate's own step-function nature."""
    df = price_df.copy()
    df["Date"] = pd.to_datetime(df["Date"])

    macro_cols = ["real_yield_10y", "dxy", "vix"]
    rate_cols = ["fed_funds_upper", "fed_funds_lower", "treasury_2y"]
    vol_cols = ["gold_implied_vol", "oil_implied_vol"]
    all_cols = macro_cols + rate_cols + vol_cols

    if not os.path.exists(macro_file):
        for col in macro_cols + ["fed_funds_midpoint", "yield_fed_spread"] + vol_cols:
            df[col] = np.nan
        df["macro_data_available"] = 0
        df["fed_rate_data_available"] = 0
        df["implied_vol_data_available"] = 0
        return df
    ...
    available_cols = [c for c in all_cols if c in macro_df.columns]
    macro_df["timestamp"] = pd.to_datetime(macro_df["timestamp"]).dt.tz_localize(None)
    macro_df = macro_df.sort_values("timestamp")[["timestamp"] + available_cols]
    ...
    present_rate_cols = [c for c in rate_cols if c in merged.columns]
    if len(present_rate_cols) == len(rate_cols):
        merged["fed_rate_data_available"] = merged[rate_cols].notna().all(axis=1).astype(int)
    else:
        for col in rate_cols:
            if col not in merged.columns:
                merged[col] = np.nan
        merged["fed_rate_data_available"] = 0

    present_vol_cols = [c for c in vol_cols if c in merged.columns]
    if len(present_vol_cols) == len(vol_cols):
        merged["implied_vol_data_available"] = merged[vol_cols].notna().all(axis=1).astype(int)
    else:
        for col in vol_cols:
            if col not in merged.columns:
                merged[col] = np.nan
        merged["implied_vol_data_available"] = 0
    ...
    for col in macro_cols + rate_cols + vol_cols:
        col_mean = merged[col].mean()
        merged[col] = merged[col].fillna(col_mean if pd.notna(col_mean) else 0.0)

    merged["fed_funds_midpoint"] = (merged["fed_funds_upper"] + merged["fed_funds_lower"]) / 2
    merged["yield_fed_spread"] = merged["treasury_2y"] - merged["fed_funds_midpoint"]

    merged = merged.drop(columns=["timestamp"])
    return merged


def compute_implied_volatility_range(annualized_iv_pct, current_price, horizon_days=1):
    """Convert a real, market-sourced ANNUALIZED implied volatility
    percentage (from GVZCLS/OVXCLS, CBOE's professional volatility
    indices computed from actual options market prices) into an honest
    expected price RANGE for a short horizon -- this is the actual
    "probabilistic range instead of a point forecast" this whole project
    was originally about.

    METHODOLOGY: standard, textbook square-root-of-time scaling --
    horizon_volatility = annualized_iv / sqrt(trading_days_per_year / horizon_days).
    This assumes 252 trading days/year (standard convention) and treats
    volatility as roughly constant over the very short horizon used here
    (1 day) -- a simplification real options traders also make for
    short horizons, not something specific or novel to this project.

    IMPORTANT FRAMING: this describes the MAGNITUDE of expected movement
    (how far price might move), NOT the DIRECTION (up or down) -- IV says
    nothing about which way price goes, only how much the market expects
    it to move either way. This is genuinely different information from
    the direction/price models elsewhere in this pipeline, and should
    never be conflated with them."""
    if annualized_iv_pct is None or current_price is None:
        return None
    horizon_vol_pct = annualized_iv_pct * np.sqrt(horizon_days / 252)
    move_dollars = current_price * (horizon_vol_pct / 100)
    return {
        "annualized_implied_volatility_pct": round(float(annualized_iv_pct), 2),
        "horizon_days": horizon_days,
        "implied_move_pct_this_horizon": round(float(horizon_vol_pct), 3),
        "implied_price_range_low": round(float(current_price - move_dollars), 2),
        "implied_price_range_high": round(float(current_price + move_dollars), 2),
        "methodology_note": (
            "Derived from a REAL, professional volatility index (CBOE's Gold/Oil ETF Volatility "
            "Index, computed from actual options market prices, same methodology as VIX) using "
            "standard square-root-of-time scaling. This is a ONE-STANDARD-DEVIATION range (roughly "
            "a 68% likelihood the actual price stays within it, under normal assumptions) -- NOT a "
            "guarantee, and NOT a directional signal. It says how far price might move, not which way."
        ),
    }
...
def load_fed_rate_outlook():
    ...
    SHARED with dxy_predictor_updater.py, which imports this module."""
...
def get_current_fed_funds_range():
    ...
    SHARED with dxy_predictor_updater.py."""
    if not os.path.exists(MACRO_FILE):
        return None, None
    with open(MACRO_FILE) as f:
        records = json.load(f)
...
def get_market_rate_expectation_proxy():
    """HONEST APPROXIMATION, not real market-implied probability data
    (e.g. CME FedWatch, which has no free/legal API this project can
    use). This uses the well-established, legitimate technique of
    comparing the 2-Year Treasury yield (DGS2, free from FRED) against
    the current Fed funds rate midpoint: when the 2yr yield sits
    meaningfully BELOW the current funds rate, markets are broadly
    pricing in future cuts over the yield's ~2-year horizon; meaningfully
    ABOVE suggests hikes are being priced in. This is a real, widely-used
    signal professional traders actually watch -- but it is a rough,
    directional proxy over a ~2-year horizon, NOT a precise per-meeting
    probability the way CME FedWatch is. Always labeled as an
    approximation in the returned fields, never presented as if it were
    the real thing. SHARED with dxy_predictor_updater.py.

    THRESHOLD CHOICE: +-0.15 percentage points is a deliberately
    conservative band -- small spreads within that range are treated as
    "roughly neutral" rather than over-interpreting minor day-to-day
    yield noise as a meaningful market signal."""
    if not os.path.exists(MACRO_FILE):
        return None
    with open(MACRO_FILE) as f:
        records = json.load(f)
    ...
    for rec in reversed(records):
        if rec.get("treasury_2y") is not None:
            latest_2y = float(rec["treasury_2y"])
            break
    fed_lower, fed_upper = get_current_fed_funds_range()
    if latest_2y is None or fed_lower is None or fed_upper is None:
        return None

    fed_midpoint = (fed_lower + fed_upper) / 2
    spread = latest_2y - fed_midpoint

    if spread < -0.15:
        lean = "cuts_appear_priced_in"
    elif spread > 0.15:
        lean = "hikes_appear_priced_in"
    else:
        lean = "roughly_neutral"

    return {
        "latest_2y_treasury_yield_pct": latest_2y,
        "fed_funds_midpoint_pct": round(fed_midpoint, 3),
        "yield_minus_fed_funds_spread_pct": round(spread, 3),
        "market_rate_expectation_lean": lean,
        "methodology_note": (
            "This is an APPROXIMATION using the 2-Year Treasury yield vs. the current Fed funds "
            "midpoint -- a real, legitimate technique, but a rough directional signal over a ~2-year "
            "horizon, NOT precise per-meeting market-implied probability (which would require a paid "
            "data source like CME FedWatch that this system does not have access to). Never present "
            "this as if it were exact market odds -- frame it as 'bond market pricing broadly suggests', "
            "not as a specific probability."
        ),
    }
...
FEATURE_COLS = ["gld_ret_1d", "gld_ret_3d", "gld_ret_5d", "gld_ma_ratio", "gld_vol10", "rsi14",
                 ...
                 "gold_implied_vol", "implied_vol_data_available",
...
def export_tree(tree):
    """Serializes one sklearn DecisionTreeRegressor's internal tree_
    structure into plain JSON-friendly lists. Uses only public,
    documented attributes (tree_.feature/threshold/children_left/
    children_right/value) -- verified byte-for-byte against sklearn's
    own predict_proba()/predict() output before this was written."""
    return {
...
def export_model_artifacts(scaler, feature_cols,
                            direction_model, direction_model_type, y_train_direction,
                            price_model, price_model_type, y_train_price,
                            sample_weight):
    """Serializes everything needed to run this exact trained model's
    inference OUTSIDE Python (in the Node backend, in JS) -- the actual
    math, not just its output. Verified exact-match against sklearn for
    all four cases (logistic/linear/GB classifier/GB regressor, weighted
    and unweighted) before this function was written; see the session
    notes for that verification. Lets a live gold price genuinely flow
    through the real model's real decision process on request, instead
    of only being available once an hour."""
    artifacts = {
        "scaler": {
    ...
    if direction_model_type == "logistic_regression":
        artifacts["direction_model"] = {
            "type": "logistic_regression",
            "coef": direction_model.coef_[0].tolist(),
            "intercept": float(direction_model.intercept_[0]),
        }
    else:  # gradient_boosting
        p_weighted = float(np.average(y_train_direction, weights=sample_weight))
        p_weighted = min(max(p_weighted, 1e-6), 1 - 1e-6)  # guard against log(0) at the extremes
        artifacts["direction_model"] = {
            "type": "gradient_boosting",
            "learning_rate": direction_model.learning_rate,
            "init_log_odds": float(np.log(p_weighted / (1 - p_weighted))),
            "trees": [export_tree(est.tree_) for est in direction_model.estimators_[:, 0]],
        }

    if price_model_type == "linear_regression":
        artifacts["price_model"] = {
            "type": "linear_regression",
            "coef": price_model.coef_.tolist(),
            "intercept": float(price_model.intercept_),
        }
    else:  # gradient_boosting
        init_value = float(np.average(y_train_price, weights=sample_weight))
        artifacts["price_model"] = {
            "type": "gradient_boosting",
            "learning_rate": price_model.learning_rate,
            "init_value": init_value,
            "trees": [export_tree(est.tree_) for est in price_model.estimators_[:, 0]],
        }
...
def run_prediction_pipeline(history_df):
    ...
    if len(df_model) < MIN_ROWS_FOR_PREDICTION:
        return {
            ...
            "latest_gold_implied_volatility_pct": None,
            "implied_vol_data_currently_available": False,
            "implied_volatility_expected_range": None,
    ...
    latest_gold_implied_vol = float(latest_row["gold_implied_vol"].values[0])
    latest_implied_vol_available = bool(latest_row["implied_vol_data_available"].values[0])
    implied_vol_range = (
        compute_implied_volatility_range(latest_gold_implied_vol, current_price, horizon_days=1)
        if latest_implied_vol_available else None
    )
    ...
    model_artifacts = export_model_artifacts(
        scaler, FEATURE_COLS,
        model, model_type_used, y_train,
        reg_model, price_model_type_used, train_df["next_ret"],
        sample_weight,
    )
    ...
    return {
        ...
        "fed_rate_data_note": (
            "The Fed funds rate midpoint and 2yr-yield spread ARE now used as real inputs to this "
            "gold model (rates are a well-established gold price driver -- lower real rates generally "
            "support gold). For the full, detailed Fed rate picture (current target range, the Fed's "
            "own published rate outlook, and the market-expectation proxy), use get_dxy_prediction "
            "instead -- that tool owns the complete, authoritative version of this data; these two "
            "fields here are just what gold's own model factors in, not the full picture."
        ),
        "latest_gold_implied_volatility_pct": latest_gold_implied_vol if latest_implied_vol_available else None,
        "implied_vol_data_currently_available": latest_implied_vol_available,
        "implied_volatility_expected_range": implied_vol_range,
        "implied_volatility_note": (
            "implied_volatility_expected_range is a REAL, market-derived expected price RANGE (from "
            "CBOE's Gold ETF Volatility Index, computed from actual GLD options prices, same "
            "methodology as VIX) -- this describes how FAR price might move over the next period, "
            "NOT which direction. It's genuinely different information from the direction/price "
            "predictions above (which are this system's own model outputs) -- never conflate the two, "
            "and never claim the range implies a direction."
        ),
        ...
        "_model_artifacts": model_artifacts,  # popped off and saved to a separate file in main() -- never meant to end up in gold_prediction_latest.json itself
    }
...
ROLLING_WINDOW = 100  # how many recent resolved predictions to summarize
...
def check_cross_asset_consistency(pred_class, asset_name="Gold"):
    """Gold/Oil and the US Dollar Index are usually (not always -- correlation
    is strong but real-world exceptions happen) inversely related: a
    stronger dollar tends to make dollar-priced commodities more expensive
    for foreign buyers, pushing them down, and vice versa. This reads
    DXY's own latest prediction (already committed to this same repo by
    the separate DXY workflow) and flags it when this asset's and DXY's
    direction calls point the SAME way -- which would contradict that
    typical relationship, and is worth an extra caution flag, not treated
    as proof either model is wrong (both models are independently weak/
    unproven regardless -- this is a plausibility cross-check, not a
    correction mechanism). SHARED across gold_predictor_updater.py and
    oil_predictor_updater.py (asset_name parameterizes which commodity
    this is being called for, so the note text is accurate either way).

    HONEST STALENESS NOTE: gold/oil update hourly/daily respectively, DXY
    every 6 hours -- so this compares against whatever DXY's most recent
    run happened to be, which can be several hours old relative to this
    asset's own fresh run. That's disclosed via dxy_data_age_hours in the
    returned dict, the same "may be stale, here's how stale" honesty
    pattern used elsewhere in this project (news sentiment, macro data)."""
    if not os.path.exists(DXY_PREDICTION_FILE):
        return {
            "cross_asset_consistency_with_dollar": "not_available",
            "cross_asset_consistency_note": "No DXY prediction data found yet to cross-check against.",
        }
    try:
        with open(DXY_PREDICTION_FILE) as f:
            dxy_data = json.load(f)
    except (json.JSONDecodeError, OSError):
        return {
            "cross_asset_consistency_with_dollar": "not_available",
            "cross_asset_consistency_note": "DXY prediction data could not be read.",
        }

    dxy_pred_class = dxy_data.get("prediction")
    if dxy_pred_class not in ("up", "down"):
        # Covers DXY's "insufficient_data" ramp-up period, or missing field
        return {
            "cross_asset_consistency_with_dollar": "not_available",
            "cross_asset_consistency_note": (
                "The DXY prediction model does not yet have a real direction call to compare "
                "against (commonly because it's still in its data ramp-up period)."
            ),
        }

    dxy_updated_at = dxy_data.get("updated_at")
    dxy_age_hours = None
    if dxy_updated_at:
        try:
            dxy_dt = datetime.fromisoformat(dxy_updated_at)
            dxy_age_hours = round((datetime.now(timezone.utc) - dxy_dt).total_seconds() / 3600, 1)
        except ValueError:
            pass

    if pred_class == dxy_pred_class:
        status = "contradicts_typical_inverse_relationship"
        note = (
            f"{asset_name}'s direction call ({pred_class}) and the Dollar Index's direction call "
            f"({dxy_pred_class}) point the SAME way -- {asset_name.lower()} and the dollar are usually "
            f"inversely related, so this is a plausibility flag worth extra caution, not proof either "
            f"model is wrong (both are independently weak/unproven signals regardless)."
        )
    else:
        status = "consistent_with_typical_inverse_relationship"
        note = (
            f"{asset_name}'s direction call ({pred_class}) and the Dollar Index's direction call "
            f"({dxy_pred_class}) point opposite ways, consistent with their typical inverse "
            f"relationship -- a mild plausibility check passed, not independent confirmation either "
            f"model is actually correct."
        )

    return {
        "cross_asset_consistency_with_dollar": status,
        "cross_asset_consistency_note": note,
        "dxy_prediction_direction": dxy_pred_class,
        "dxy_data_age_hours": dxy_age_hours,
    }
...
def main():
    ...
    # Cross-check against DXY's latest prediction, if gold actually made a
    # real direction call (skip for insufficient_data -- nothing to check).
    if result.get("prediction") in ("up", "down"):
        cross_asset_stats = check_cross_asset_consistency(result["prediction"])
        result.update(cross_asset_stats)

    model_artifacts = result.pop("_model_artifacts", None)
    ...
    print(f"Saved {PREDICTION_FILE}, {HISTORY_FILE}, {TRACK_RECORD_FILE}, and {MODEL_ARTIFACTS_FILE} locally -- "
