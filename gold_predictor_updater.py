...
# (No API key config needed -- Binance's public price endpoint requires none.)
...
def fetch_live_price():
    """Fetch the current PAXG/USDT price from Binance's public market data
    API -- the same source used for the historical backfill (see
    backfill_paxg_history.py), so live and historical data now share
    exactly one source and one unit, permanently avoiding the kind of
    mismatch bug this project hit earlier. This endpoint is public and
    free with no meaningful rate limit for this volume of use, unlike
    GoldAPI's 100-requests/month cap, which is why the update schedule
    can now run hourly instead of every 8 hours."""
