...
def check_cross_asset_consistency(gold_pred_class):
    """Gold and the US Dollar Index are usually (not always -- correlation
    is strong but real-world exceptions happen) inversely related: a
    stronger dollar tends to make dollar-priced gold more expensive for
    foreign buyers, pushing gold down, and vice versa. This reads DXY's
    own latest prediction (already committed to this same repo by the
    separate DXY workflow) and flags it when gold's and DXY's direction
    calls point the SAME way -- which would contradict that typical
    relationship, and is worth an extra caution flag, not treated as
    proof either model is wrong (both models are independently weak/
    unproven regardless -- this is a plausibility cross-check, not a
    correction mechanism).

    HONEST STALENESS NOTE: gold updates hourly, DXY every 6 hours -- so
    this compares against whatever DXY's most recent run happened to be,
    which can be up to ~6 hours old relative to gold's own fresh run.
    That's disclosed via dxy_data_age_hours in the returned dict, the
    same "may be stale, here's how stale" honesty pattern used elsewhere
    in this project (news sentiment, macro data)."""
    ...
    if gold_pred_class == dxy_pred_class:
        status = "contradicts_typical_inverse_relationship"
        note = (
            f"Gold's direction call ({gold_pred_class}) and the Dollar Index's direction call "
            f"({dxy_pred_class}) point the SAME way -- gold and the dollar are usually inversely "
            f"related, so this is a plausibility flag worth extra caution, not proof either model "
            f"is wrong (both are independently weak/unproven signals regardless)."
        )
    else:
